...
/**
 * Represents a sprite sheet composed of evenly sized frames arranged
 * in a grid (rows × columns). This class provides:
 *
 * <ul>
 *     <li>Automatic calculation of each frame's width and height</li>
 *     <li>Rendering of any frame index to a GraphicsContext</li>
 *     <li>Support for customizable animation sequences</li>
 *     <li>Methods for advancing to the next frame in the sequence</li>
 * </ul>
 *
 * Sprite indices are numbered from 0 → (spriteCount - 1),
 * scanning across rows first (left-to-right, then top-to-bottom).
 *
 * This class does not perform timing; callers should advance frames
 * using an animation loop such as AnimationTimer or Timeline.
 */
...
public class Sprite {
    ...
    /**
     * Ordered list of frame indices defining the animation sequence.
     * Example: [0, 1, 2, 1] makes a smoother looping walk cycle.
     */
   ...
   /**
     ...
     *
     * @param resourcePath  the resource path (path after /src/main/resources/... )
     *                      use linux style paths for windows & linux
     * @param rows          number of rows the sheet is divided into
     * @param cols          number of columns the sheet is divided into
     * @param spriteCount   total number of valid frames in the sheet
     *                      (supports sheets where the last row is partially filled)
     *
    ...
    /**
     * Replace the current animation sequence with a custom one.
     * All frame indices must be between 0 and spriteCount-1.
     *
